...
class HyperRAMPacketFIFO(Elaboratable):
    def __init__(self, out_fifo_depth=None, ram_bus=None, shared_psram=None):
        ...
        self.out_fifo_depth = (
            max(out_fifo_depth, 2) if out_fifo_depth is not None else 2
        )
        self.ram_bus = ram_bus  # Allow passing in a pre-requested RAM resource
        self.shared_psram = (
            shared_psram  # Allow passing a shared psram controller interface
        )

    def elaborate(self, platform):
        m = Module()

        # HyperRAM submodules - either use provided shared PSRAM or create a new one
        if self.shared_psram is None:
            if self.ram_bus is None:
                try:
                    ram_bus = platform.request("ram")
                except Exception as e:
                    print(
                        f"WARNING: Failed to request RAM resource: {e}. HyperRAM functionality will be limited."
                    )
                    # Create a minimal dummy implementation
                    return self._create_dummy_implementation(m)
            else:
                ram_bus = self.ram_bus

            # Create new PSRAM controller
            psram_phy = HyperRAMPHY(bus=ram_bus)
            psram = HyperRAMInterface(phy=psram_phy.phy)
            m.submodules += [psram_phy, psram]
        else:
            # Use the shared PSRAM controller
            psram = self.shared_psram
            ram_bus = self.ram_bus
        ...
        m.submodules.out_fifo = out_fifo = SyncFIFOBuffered(
            width=16, depth=self.out_fifo_depth
        )

        # Hook up our PSRAM.
        # For shared PSRAM, only the first module should drive control signals
        if self.shared_psram is None:
            # When using our own PSRAM controller, we need to drive all signals
            m.d.comb += [
            ...
            # When using shared PSRAM controller, only connect data paths
            # and avoid driving control signals that would cause conflicts
            m.d.comb += [
                psram.write_data.eq(self.input.payload),
                self.input.ready.eq(psram.write_ready),
                # Wire PSRAM -> output FIFO -> output stream
        ...
        with m.If(is_write):
            ...
            m.d.comb += psram.final_word.eq(
                (word_count == (depth - 1)) | self.input.last
            )
        with m.Else():
            # READ: Finish when PSRAM is empty or the output FIFO is full.
            m.d.comb += psram.final_word.eq(
                (word_count == 1) | (out_fifo.level == out_fifo.depth - 1)
            )
    ...
    def _create_dummy_implementation(self, m):
        """Create a dummy implementation when RAM is not available"""
        # Create a small FIFO to provide minimal passthrough functionality
        m.submodules.fallback_fifo = fallback_fifo = SyncFIFOBuffered(
            width=16, depth=self.out_fifo_depth
        )

        # Wire input directly to output through the small FIFO
        m.d.comb += [
            fallback_fifo.w_data.eq(self.input.payload),
            fallback_fifo.w_en.eq(self.input.valid),
            self.input.ready.eq(fallback_fifo.w_rdy),
            self.output.payload.eq(fallback_fifo.r_data),
            self.output.valid.eq(fallback_fifo.r_rdy),
            fallback_fifo.r_en.eq(self.output.ready),
        ]
...
class AsyncFIFOReadReset(Elaboratable, FIFOInterface):
    def __init__(
        self, *, width, depth, r_domain="read", w_domain="write", exact_depth=False
    ):
        if depth != 0:
            try:
    ...
    def elaborate(self, platform):
        ...
        produce_cdc = m.submodules.produce_cdc = FFSynchronizer(
        ...
        consume_cdc = m.submodules.consume_cdc = FFSynchronizer(
