...
# Verify Yosys version >= 0.23
yosys_version=$(yosys -V | awk '{print $2}')
[[ "$yosys_version" < "0.23" ]] && { err "Error: Yosys >=0.23 required (found $yosys_version)"; exit 1; }
...
YOSYS_SCRIPT="$BUILD_DIR/synth.ys"
{
  ...
  echo "verilog_defaults -push"
  echo "verilog_defaults -add -DSYNTHESIS"
  find "$RTL_DIR" -name '*.v' -exec echo "read_verilog -sv -I $RTL_DIR/include {}" \;
  echo "verilog_defaults -pop"
  echo "hierarchy -generate -top $TOP_MODULE"
  echo "synth_ecp5 -abc9 -dff -goal area -json $SYNTH_JSON"
} > "$YOSYS_SCRIPT"
...
if grep -q "Warning:" "$LOG_DIR/synthesis.log"; then
  err "Synthesis warnings detected - check $LOG_DIR/synthesis.log"
  exit 1
fi
